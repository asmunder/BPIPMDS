...
and the references therein (see however section \ref{sec:bugs}). For research
...
interfacial properties, it is possible to state that the Verlet leapfrog algorithm is commonly used
...
\begin{figure}
	...
	\end{subfigure}
\caption{Diagonal elements of the pressure tensor ${\langle}P_{xx}{\rangle}$, ${\langle}P_{yy}{\rangle}$, and ${\langle}P_{zz}{\rangle}$ and the resulting interfacial tension, ${\gamma}$, evaluated over the volume simulation box for the case of CO$_{2}$ (single Mie sphere, $\epsilon / k_B = 353.55 \rm{K}; \sigma = 3.741 \AA; \lambda_r = 23.0; \lambda_a = 6.66$) at 240 K.
}
\label{fig:16}
\end{figure}
...
shape to persist \citep{lau2015}. For further details concern to metastable states,
